...
import time
import traceback
import xml.etree.ElementTree as ET  # nosec: B405 - used only for parsing trusted internal evaluation XML files
...
def parse_evaluation_file(file_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        tree = ET.parse(
            file_path
        )  # nosec: B314 - evaluation files are trusted local assets
        root = tree.getroot()
        evaluations = []
