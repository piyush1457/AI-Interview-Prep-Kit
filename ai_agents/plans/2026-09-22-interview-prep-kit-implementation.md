...
| Pipeline | Job+SSE, `runPipeline()` shared by API + CLI + batch-upload; Groq llama-3.1-8b-instant via shared PQueue singleton (concurrency 1 + 500ms across all jobs, tracks queueWaitMs per case), 45s/call, 3x backoff, max 2 coverage passes, wall 2.5min + queueWaitMs per case (v2 fix A, PIPELINE_TIMEOUT on exceed), partial-honest ok on exceed |
